...
**macOS:**
- `Project.xcconfig` — Xcode build settings and paths to openFrameworks core
- `openFrameworks-Info.plist` — App bundle metadata and permissions
- `of.entitlements` — Security permissions (camera, USB, audio)
- **Note:** AGL.framework must be removed from Xcode's linked libraries (deprecated framework)

**Windows:**
- `.sln` and `.vcxproj` files generated by projectGenerator
- Uses Visual Studio 2022

**Source Files:**
- `src/DisplayManager.cpp/h` — Core logic for webcam, proximity detection, video playback
- `src/DisplayApp.cpp/h` — Per-window renderer
- `src/main.cpp` — Multi-window initialization
- `bin/data/haarcascade_frontalface_default.xml` — Face detection model
- `bin/data/movies/` — Video content (not in git)
...
# Press Ctrl+Shift+B or F5 to run
```
